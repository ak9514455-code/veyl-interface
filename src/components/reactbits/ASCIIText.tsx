...
class CanvAscii {
  ...
  setRenderer() {
    ...
    this.container.addEventListener("touchmove", this.onMouseMove as any);
  }
  ...
  dispose() {
    ...
    this.container.removeEventListener("touchmove", this.onMouseMove as any);
    this.clear();
    if (this.renderer) {
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const setup = async () => {
          ...
          async ([entry]) => {
            ...
            if (entry.isIntersecting && r.width > 0 && r.height > 0) {
              ...
              if (!cancelled && containerRef.current) {
                asciiRef.current = await createAndInit(containerRef.current, r.width, r.height);
                if (!cancelled) asciiRef.current?.load();
                else asciiRef.current?.dispose();
              }
            }
          },
          { threshold: 0.1 },
      ...
      const instance = await createAndInit(el, width, height);
      if (cancelled) {
        instance.dispose();
        return;
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`ascii-text-container relative h-full w-full ${className}`}
